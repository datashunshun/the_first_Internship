...
condition_1 = 'mt_tag_objecttag_entry.entry_blog_id = 7 OR mt_tag_objecttag_entry.entry_blog_id = 11'
condition_2 = 'mt_tag_objecttag_entry.entry_status  = 2'
condition_3 = 'mt_tag_objecttag_entry.entry_id >= 50000'

COLUMNS = 'entry_id,entry_title,entry_text,entry_text_more,entry_blog_id,entry_status,tag_id,tag_name'

query_1 = f'SELECT {COLUMNS} FROM mt_tag_objecttag_entry where {condition_1} AND {condition_2} AND {condition_3}'
...
df_id_tag_groupby = df.groupby('entry_id')['tag_name'].agg(','.join)
df_id_groupby_pre = df.drop_duplicates(subset = 'entry_id')
df_id_groupby = df_id_groupby_pre.loc[:,['entry_id','entry_title','entry_text','entry_text_more','entry_blog_id','entry_status']]

df_id_tag_groupby.reset_index(drop=True, inplace=True)
df_id_groupby.reset_index(drop=True, inplace=True)

df_id_tag_groupby.to_csv('id_tag_groupby.csv')
df_id_groupby.to_csv('id_groupby.csv')

N = int(len(df_id_groupby['entry_id']))
print(N)

drop_index = []

for i in range(0,N):
    if(pd.isnull(df_id_groupby['entry_text'][i]) ==True and pd.isnull(df_id_groupby['entry_text_more'][i]) == True):
        drop_index.append(i)
df_id_groupby.to_csv('df_id_groupby.csv')
df_id_tag_groupby.to_csv('df_id_tag_groupby.csv')

df_id_groupby = df_id_groupby.drop(drop_index)
df_id_tag_groupby = df_id_tag_groupby.drop(drop_index)

M = int(len(df_id_groupby['entry_text']))

df_id_groupby = df_id_groupby.reset_index()
df_id_tag_groupby = df_id_tag_groupby.reset_index()

print(df_id_groupby['entry_text'].isnull().sum())
print(df_id_groupby['entry_text_more'].isnull().sum())
print(df_id_tag_groupby['tag_name'].isnull().sum())

out_entry_text = [0]*M

for i in range(0,M):
    if((pd.notnull(df_id_groupby['entry_text'][i]) == True) and (pd.notnull(df_id_groupby['entry_text_more'][i]) == True)):
        out_entry_text[i] = df_id_groupby['entry_text'][i] + df_id_groupby['entry_text_more'][i] + df_id_tag_groupby['tag_name'][i]
    elif(pd.notnull(df_id_groupby['entry_text'][i]) == True):
        out_entry_text[i] = df_id_groupby['entry_text'][i] + df_id_tag_groupby['tag_name'][i]
    else:
        out_entry_text[i] = df_id_groupby['entry_text_more'][i] +  df_id_tag_groupby['tag_name'][i]
